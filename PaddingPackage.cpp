...
PaddingPackage::PaddingPackage(const Encoder& e) : index(count), rsize(0), psize(e.getk() * e.getl()), pdata(psize) {
    if (count < 65535) {
        count++;
    } else {
        count = 0;
...
PaddingPackage::PaddingPackage(const Encoder& e, const char* rdata, size_t rsize)
...
std::pair<char*, size_t> PaddingPackage::getRawData() { return std::make_pair(pdata.data() + 4, rsize); }

size_t PaddingPackage::getindex() { return index; }
